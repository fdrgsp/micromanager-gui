...
For GPU acceleration (Windows/Linux only), combine a cellpose extra with a CUDA extra (`cu126`, `cu128`, or `cu130`) and pass the matching PyTorch index so the CUDA wheels are found:
...
```bash
uv pip install "git+https://github.com/fdrgsp/micromanager-gui[cp4,cu128]" --extra-index-url https://download.pytorch.org/whl/cu128
```
