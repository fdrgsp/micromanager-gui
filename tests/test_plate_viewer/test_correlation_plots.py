...
class TestCorrelationPlots:
    ...
    def test_calculate_cross_correlation_basic(self, sample_roi_data_active):
        ...
        correlation_matrix, rois_idxs = _calculate_cross_correlation(sample_roi_data_active)
        ...
        np.testing.assert_array_almost_equal(np.diag(correlation_matrix), [1.0, 1.0, 1.0], decimal=2)
        
        # Check that matrix is symmetric
        np.testing.assert_array_almost_equal(correlation_matrix, correlation_matrix.T, decimal=10)
    ...
    def test_calculate_cross_correlation_inactive_rois(self, sample_roi_data_inactive):
        ...
        correlation_matrix, rois_idxs = _calculate_cross_correlation(sample_roi_data_inactive)
    ...
    @patch('micromanager_gui._plate_viewer._plot_methods._single_wells_plots._correlation_plots._add_hover_functionality_cross_corr')
    def test_plot_cross_correlation_data_success(self, mock_hover, mock_widget, sample_roi_data_active):
    ...
    def test_plot_cross_correlation_data_insufficient_rois(self, mock_widget, single_roi_data):
    ...
    def test_plot_cross_correlation_data_with_roi_filter(self, mock_widget, sample_roi_data_active):
    ...
    @patch('micromanager_gui._plate_viewer._plot_methods._single_wells_plots._correlation_plots._plot_hierarchical_clustering_map')
    def test_plot_hierarchical_clustering_data_map_mode(self, mock_plot_map, mock_widget, sample_roi_data_active):
        """Test hierarchical clustering plotting in map mode."""
        _plot_hierarchical_clustering_data(mock_widget, sample_roi_data_active, use_dendrogram=False)
    ...
    @patch('micromanager_gui._plate_viewer._plot_methods._single_wells_plots._correlation_plots._plot_hierarchical_clustering_dendrogram')
    def test_plot_hierarchical_clustering_data_dendrogram_mode(self, mock_plot_dendro, mock_widget, sample_roi_data_active):
        """Test hierarchical clustering plotting in dendrogram mode."""
        _plot_hierarchical_clustering_data(mock_widget, sample_roi_data_active, use_dendrogram=True)
    ...
    def test_plot_hierarchical_clustering_data_insufficient_rois(self, mock_widget, single_roi_data):
    ...
    def test_plot_hierarchical_clustering_dendrogram(self):
        ...
        correlation_matrix = np.array([[1.0, 0.8, 0.6], [0.8, 1.0, 0.7], [0.6, 0.7, 1.0]])
        rois_idxs = [1, 2, 3]
        ...
        assert ax.get_title() == "Pairwise Cross-Correlation (Hierarchical Clustering Dendrogram)"
        assert ax.get_ylabel() == "Distance"

    @patch('micromanager_gui._plate_viewer._plot_methods._single_wells_plots._correlation_plots._add_hover_functionality_clustering')
    @patch('scipy.cluster.hierarchy.leaves_list')
    @patch('scipy.cluster.hierarchy.linkage')
    @patch('scipy.spatial.distance.squareform')
    def test_plot_hierarchical_clustering_map(self, mock_squareform, mock_linkage, mock_leaves_list, mock_hover, mock_widget):
        ...
        _plot_hierarchical_clustering_map(mock_widget, mock_ax, correlation_matrix, rois_idxs)
    ...
    def test_add_hover_functionality_clustering(self, mock_cursor, mock_widget):
        ...
        _add_hover_functionality_clustering(mock_image, mock_widget, rois, order, values)
    ...
    def test_correlation_with_identical_traces(self):
        ...
        assert correlation_matrix is not None
        np.testing.assert_array_almost_equal(correlation_matrix, [[1.0, 1.0], [1.0, 1.0]], decimal=2)
    ...
    @patch('micromanager_gui._plate_viewer._plot_methods._single_wells_plots._correlation_plots._calculate_cross_correlation')
