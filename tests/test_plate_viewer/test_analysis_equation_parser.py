...
from micromanager_gui._plate_viewer._analysis import _AnalyseCalciumTraces


class TestEquationParser:
    """Test the equation_from_str method in _AnalyseCalciumTraces."""

    @pytest.fixture
    def analysis_widget(self, qapp):
        """Create an analysis widget with proper Qt application setup."""
    ...
    def test_linear_equations(self, analysis_widget: _AnalyseCalciumTraces):
        ...
        func = analysis_widget.equation_from_str("y = 11.07*x - 6.63")
        assert func is not None
        assert abs(func(1) - 4.44) < 0.01  # 11.07*1 - 6.63 = 4.44

        # Linear equation with negative slope
        func = analysis_widget.equation_from_str("y = -0.5*x + 10")
        assert func is not None
    ...
    def test_quadratic_equations(self, analysis_widget: _AnalyseCalciumTraces):
        ...
        func = analysis_widget.equation_from_str("y = 0.5*x^2 + 1*x + 2")
        assert func is not None
    ...
    def test_exponential_equations(self, analysis_widget: _AnalyseCalciumTraces):
        ...
        func = analysis_widget.equation_from_str("y = 1*exp(0.1*x) + 0")
        assert func is not None
        result = func(0)
    ...
    def test_power_equations(self, analysis_widget: _AnalyseCalciumTraces):
        ...
        # Square
        func = analysis_widget.equation_from_str("y = 1*x^2 + 0")
        assert func is not None
        assert func(3) == 9  # 1*3^2 + 0 = 9

        # Cube
        func = analysis_widget.equation_from_str("y = 0.5*x^3 + 1")
        assert func is not None
    ...
    def test_logarithmic_equations(self, analysis_widget: _AnalyseCalciumTraces):
        ...
        func = analysis_widget.equation_from_str("y = 2*log(x) + 1")
        assert func is not None
        result = func(np.e)
    ...
    def test_empty_equation(self, analysis_widget: _AnalyseCalciumTraces):
        ...
        func = analysis_widget.equation_from_str(None)
        assert func is None

    @patch("micromanager_gui._plate_viewer._analysis.show_error_dialog")
    def test_invalid_equations(
        self, mock_error_dialog, analysis_widget: _AnalyseCalciumTraces
    ):
        """Test that invalid equations return None and handle errors gracefully."""
        # Missing coefficient
        func = analysis_widget.equation_from_str("y = x")
        assert func is None

        # Missing multiplication sign
        func = analysis_widget.equation_from_str("y = 2x + 3")
        assert func is None

        # Completely invalid
        func = analysis_widget.equation_from_str("invalid equation")
        assert func is None

        # Invalid format
        func = analysis_widget.equation_from_str("x = 2*y + 3")
        assert func is None

        # Missing operator
        func = analysis_widget.equation_from_str("y = 2 3")
        assert func is None

        # Verify that error dialogs were called for invalid equations
        assert mock_error_dialog.call_count == 5

    def test_case_insensitive_parsing(self, analysis_widget: _AnalyseCalciumTraces):
        """Test that equation parsing is case insensitive."""
        # Uppercase
        func = analysis_widget.equation_from_str("Y = 2*X + 3")
        assert func is not None
        assert func(5) == 13
    ...
    def test_edge_cases(self, analysis_widget: _AnalyseCalciumTraces):
        """Test edge cases and boundary conditions."""
        # Zero coefficients
        func = analysis_widget.equation_from_str("y = 0*x + 5")
        assert func is not None
        assert func(100) == 5  # Should always return 5

        # Very small numbers
        func = analysis_widget.equation_from_str("y = 0.001*x + 0.002")
        assert func is not None
        assert abs(func(1000) - 1.002) < 0.001

        # Very large numbers
        func = analysis_widget.equation_from_str("y = 1000*x + 2000")
        assert func is not None
        assert func(1) == 3000

    def test_array_input(self, analysis_widget: _AnalyseCalciumTraces):
        """Test that equations work with numpy arrays."""
        func = analysis_widget.equation_from_str("y = 2*x + 3")
        assert func is not None
    ...
    def test_specific_use_case_equations(self, analysis_widget: _AnalyseCalciumTraces):
        """Test equations that might be commonly used for LED power calibration."""
        ...
        # Common power curve
        func = analysis_widget.equation_from_str("y = 0.1*x^2 + 0.5*x + 1")
        assert func is not None
        assert func(10) == 16  # 0.1*100 + 0.5*10 + 1 = 10 + 5 + 1 = 16

        # Exponential growth for high-power LEDs
        func = analysis_widget.equation_from_str("y = 1*exp(0.05*x) + 0")
        assert func is not None
        result = func(0)
        assert abs(result - 1) < 0.01  # exp(0) = 1

        # Square root response (common in optical systems)
        func = analysis_widget.equation_from_str("y = 5*x^0.5 + 2")
        assert func is not None
