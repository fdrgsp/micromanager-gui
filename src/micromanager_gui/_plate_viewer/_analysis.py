...
class _AnalyseCalciumTraces(QWidget):
    ...
    def _process_roi_trace(
        ...
        # use Hilbert Transform to calculate instantaneous phases (FluoroSNNAP)
        instantaneous_phase = []
        analytic_signal = hilbert(dec_dff)
        instantaneous_phase = (np.angle(analytic_signal)).tolist()
        ...
        self._analysis_data[pos_name][str(label_value)] = ROIData(
            ...
            linear_phase=linear_phase,  # NOTE: to delete
            cubic_phase=cubic_phase,  # NOTE: to delete
