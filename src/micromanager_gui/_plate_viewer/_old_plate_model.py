...
def save_database(database: dict[str, OldPlate], database_path: Path | str) -> None:
...
def load_database(database_path: Path | str) -> dict[str, OldPlate]:
    ...
    with open(Path(database_path)) as f:
        return {k["id"]: OldPlate(**k) for k in json.load(f)}
