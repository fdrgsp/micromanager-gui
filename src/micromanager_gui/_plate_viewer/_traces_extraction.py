...
class _ExtractCalciumTraces(QWidget):
    ...
    def _extract_trace_data_per_position(self, p: int) -> None:
        ...
        for label_value, label_mask in tqdm(labels_masks.items(), desc=msg):
            ...
            self._process_roi_trace(
                data, meta, fov_name, label_value, label_mask, tot_time_sec
            )
    ...
    def _process_roi_trace(
        ...
        roi_size_pixel * (px_size**2) if px_size else roi_size_pixel
        ...
        LOGGER.info(
            f"Decay constant: {t:.3f} seconds, "
            f"Sampling frequency: {len(roi_trace) / tot_time_sec:.2f} Hz"
        )

        # convert mask to efficient coordinate representation
